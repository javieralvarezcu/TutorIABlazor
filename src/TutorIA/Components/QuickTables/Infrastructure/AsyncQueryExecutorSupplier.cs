...
namespace TutorIA.Components.QuickTables.Infrastructure;
            ...
            if (executor is null)
            {
                    ...
                    IsEntityFrameworkProviderTypeCache.GetOrAdd(providerType, IsEntityFrameworkProviderType))
                {
                    throw new InvalidOperationException(
                        $"The supplied {nameof(IQueryable)} is provided by Entity Framework. To query it efficiently, you must reference the package TutorIA.Components.QuickTables.EntityFrameworkAdapter and call AddQuickTableEntityFrameworkAdapter on your service collection.");
                }
            }
