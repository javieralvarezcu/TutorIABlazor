...
namespace IconGenerator.Tabler
{
    public static class TablerGenerator
    {
        ...
        public static async Task<List<GeneratedFlag>> GenerateFlags()
        {
            ...
            httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("TutorIA", "1"));
            ...
            foreach (var file in entries.Where(e => e.Name.EndsWith(".svg")))
            {
                ...
                if (country != null)
                {
                    generatedFlag.FlagType.Country = new TutorIA.Country(country.CommonName, country.Alpha2Code.ToString(), country.Alpha3Code.ToString(), country.NumericCode);
                }
        ...
        public static async Task<IEnumerable<GeneratedIcon>> GenerateIcons()
        {
            ...
            var spriteLocal = File.ReadAllText(@"C:\Code\Github\TutorIA\Icons\IconGenerator\Tabler\sprites\dist\tabler-sprite.svg");
            ...
            foreach (var elements in iconElements)
            {
                ...
                icon.IconType = new TutorIA.TablerIcon(Utilities.ExtractIconElements(elements.Elements()), isFilled);
                icons.Add(icon);
